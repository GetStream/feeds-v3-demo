...
import { FeedsClient, CommentResponse, AddCommentReactionResponse } from '@stream-io/feeds-client';
...
export function useComments() {
  ...
  const addCommentReaction = useCallback(async (commentId: string, type: string) => {
    if (!client) return false;
  ...
  const deleteCommentReaction = useCallback(async (commentId: string, type: string, userId: string) => {
    if (!client) return false;
      ...
      if (comment) {
        const updatedComments = comments.map(c => 
          c.id === commentId 
            ? { 
                ...c, 
                latest_reactions: c.latest_reactions?.filter(r => !(r.type === type && r.user.id === userId))
              }
            : c
        );
        setComments(updatedComments);
  ...
  const toggleCommentReaction = useCallback(async (commentId: string, type: string, userId?: string) => {
  ...
  const getUserReactionForComment = useCallback((comment: CommentResponse, type: string, userId: string) => {
